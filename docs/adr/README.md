# Architectural Decision Records — agent-nuget-pm

Naming: `ANPM-ADR-NNNN-<topic>.md`

| ADR | Status |
|-----|--------|
| [0001 Overview](ANPM-ADR-0001-overview.md) | Accepted |
| [0002 Not BaGet](ANPM-ADR-0002-not-baget-nexus-clone.md) | Accepted |
| [0003 Forge plugin boundary](ANPM-ADR-0003-forge-plugin-boundary.md) | Proposed |
| [0004 MCP tool surface](ANPM-ADR-0004-mcp-tool-surface.md) | Accepted |
| [0005 TOML config](ANPM-ADR-0005-toml-config.md) | Accepted |
